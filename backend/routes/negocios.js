...
router.get('/', async (req, res) => {
  ...
  if (error) {
    // Fallback: sin columnas opcionales que pueden no existir aún
    let q2 = supabase.from('negocios').select('id,nombre,direccion,zona,ciudad,telefono,categoria').order('nombre');
...
// GET /api/negocios/mi-negocio — el negocio del restaurante autenticado
...
// POST /api/negocios — crear negocio (restaurante)
router.post('/', authMiddleware, async (req, res) => {
  if (req.usuario.rol !== 'restaurante' && req.usuario.rol !== 'admin')
    return res.status(403).json({ error: 'No autorizado' });
  const { nombre, descripcion, direccion, zona, ciudad, telefono, categoria, email } = req.body;
  if (!nombre) return res.status(400).json({ error: 'Nombre requerido' });
  const { data, error } = await supabase
    .from('negocios')
    .insert([{ propietario_id: req.usuario.id, nombre, descripcion, direccion, zona, ciudad: ciudad || 'Guatemala', telefono, categoria, email: email || req.usuario.email }])
    .select()
    .single();
...
router.put('/:id', authMiddleware, async (req, res) => {
  const { data: negocio } = await supabase.from('negocios').select('propietario_id').eq('id', req.params.id).single();
  if (!negocio) return res.status(404).json({ error: 'Negocio no encontrado' });
  if (negocio.propietario_id !== req.usuario.id && req.usuario.rol !== 'admin')
    return res.status(403).json({ error: 'No autorizado' });
  const { nombre, descripcion, direccion, zona, ciudad, telefono, categoria, activo } = req.body;
  const updates = {};
...
router.get('/:id/estadisticas', authMiddleware, async (req, res) => {
  ...
  if (!pedidos) {
    // Fallback sin filtro por estado_pago
  ...
  res.json({
    ...
    pedidos_hoy: (pedidos || []).filter(p => { const d = p.created_at || p.creado_en; return d && new Date(d).toDateString() === new Date().toDateString(); }).length,
  });
});
